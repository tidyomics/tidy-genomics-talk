...
set.seed(5)
p <- makeClusterRanges(chrom, rng_big, n=300, lambda=5, seqlens)
...
textparams <- pgParams(x=.1, rot=90, just="left")

# plot the original GRanges, e.g. suppose ATAC-seq peaks
plotRanges(p, params=params, y=2)
plotText("original", params=textparams, y=3)

# uniform shuffling
shuf <- shuffle(p, rng_big)

# plot shuffled ranges
plotRanges(shuf, params=params, y=1)
plotText("shuffled", params=textparams, y=2)
...
set.seed(1)
boot <- bootRanges(p, blockLength=1e5, R=1,
                   seg=seg, proportionLength=FALSE)

# plot bootstrapped ranges
plotRanges(boot, params=params, y=0)
plotText("boot", params=textparams, y=1)
...
g %>%
  mutate(n_overlaps = count_overlaps(., p))
...
g %>% 
  join_overlap_left(p) %>%
  group_by(symbol) %>% # per gene symbol
...
shuf_list <- replicate(niter, shuffle(p, rng_big))
shuf_long <- bind_ranges(shuf_list, .id="iter")

boot_long <- bootRanges(p, blockLength=1e5, R=niter,
                   seg=seg, proportionLength=FALSE)
...
plotRanges(focal, params=params, y=2)
plotText("focal", params=textparams, y=3)

# make a 'pool' of features to select from
pool <- makePool(5000, chrom, rng, seqlens)

# plot the pool (subset)
plotRanges(pool[1:200], params=params, y=1)
plotText("pool", params=textparams, y=2)
...
plotRanges(pool[1:200], params=params, y=1)
plotText("pool", params=textparams, y=2)
plotRanges(matched(m), params=params, y=0)
plotText("matched", params=textparams, y=1)
