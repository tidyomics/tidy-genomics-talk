...
* Suppose a query set of ranges, `gr` (here three ranges).
* The following chunk groups `pks` by the query ranges. 
* Optional specification of `maxgap` and/or `minoverlap`. 
* Range information for the `pks` is lost though.
...
* Join-by-overlaps is a more flexible paradigm.
* "Left" and "inner" joins differ by how missing IDs in the first table are handled.
