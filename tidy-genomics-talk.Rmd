...
# Why the "tidy analysis" paradigm for genomic ranges?

* Many comp students are already familiar with dplyr/ggplot2
* Less variable names, e.g. `dat3 <- dat2[dat2$signal > x]`
* Encourages exploratory analysis, compare to "all-in-one" functions
...
gr %>%
  ...
  nest(data=-query) %>%
  mutate(model = map_dbl(data, ~lm(.x$score ~ .x$qValue)))
```
...
* BioC2022: Wancen Mu & Eric Davis present *nullranges* workshop
